...
struct Invoice: Identifiable, Codable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, invoiceNumber, clientName, clientEmail, clientAddress
        case date, dueDate, lineItems, taxRate, notes, status
        case discountType, discountValue, currency, payments
        case pdfFileName, pdfURL
    }
...
final class InvoiceStorage: ObservableObject {
    ...
    static func loadEstimates() -> [Estimate] {
        return shared.estimates
    }

    static func loadRecurringInvoices() -> [RecurringInvoice] {
        return shared.recurringInvoices
    }

    static func loadItemTemplates() -> [ItemTemplate] {
        return shared.items
    }

    static func replaceAllInvoices(_ invoices: [Invoice]) {
        shared.invoices = invoices
        shared.saveData()
    }

    static func replaceAllClients(_ clients: [Client]) {
        shared.clients = clients
        shared.saveData()
    }

    static func replaceAllEstimates(_ estimates: [Estimate]) {
        shared.estimates = estimates
        shared.saveData()
    }

    static func replaceAllRecurringInvoices(_ recurring: [RecurringInvoice]) {
        shared.recurringInvoices = recurring
        shared.saveData()
    }

    static func replaceAllItemTemplates(_ items: [ItemTemplate]) {
        shared.items = items
        shared.saveData()
    }

    static func saveBusinessInfo(_ info: BusinessInfo) {
        shared.businessInfo = info
        shared.saveData()
    }

    static func getInvoicesDirectory() -> URL {
        let documentsPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        let invoicesDir = documentsPath.appendingPathComponent("Invoices", isDirectory: true)
